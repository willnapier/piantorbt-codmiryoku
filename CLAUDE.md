...
- **Ctrl+Z hard no-op** (macOS keymap, 2026-06-28): base-layer Z is a `z_no_ctrl` mod-morph — Z alone = "z", but Z while **Left-Ctrl** is held emits nothing. Reads live modifier state, so it neutralises a *deliberately-held* A (=LCtrl) followed by Z — which the positional `hml` guard above cannot, because a sustained hold engages the mod by design. Kills terminal-suspend (SIGTSTP) at the source. **Cmd+Z (LGUI) undo is unaffected** (only Left-Ctrl is in the trigger mask). macOS keymap only; Linux keymap unchanged.
